  ...
  socket.onmessage = function(message){
	...
	$('.smplBtn').on('click', function(){
		...
		if(id=="smpl1"){
			...
			$("#text").val("電源投入不可、電源を入れようとすると青点滅になるが電源は入らない。　◎発熱有。フィルム破棄了承なし。しばらくすると電源が入りAndroid起動中画面になり（相当時間がかかる）、電源投入可能になる。この症状が頻繁に起こる。")
		}else{
			$("#text").val("フタを閉じたり開けたりすると電源ＯＦＦという表示とともに電源が切れる。電源投入も反応が悪いのか入れづらい状況")
		}
	});
